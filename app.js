...
app.post("/checkReferral", async function(req, res) {
  if (
    !req.body.referralIdentity ||
    !req.body.email ||
    req.body.apiKey !== apiKey
  ) {
    logger.warning(
      `checkReferral request not valid: referralIdentity: ${req.body
        .referralIdentity} email: ${req.body.email} api: ${req.body.apiKey}`
    );
    res.status(400);
    res.send(JSON.stringify({ errorCode: errorCodes.inputParamsNotValid }));
    return;
  ...
  try {
    let branchIdentity = await branchClient.getBranchIdentity(
      req.body.referralIdentity
    );

    if (!branchIdentity) {
      logger.warning(
        `Branch identi was not found:  ${req.body.referralIdentity}`
      );
      res.status(400);
    ...
    if (!emailIsValid || !refferalEmailIsValid) {
      logger.warning(
        `checkReferral email validation failed [Email: ${email} valid: ${emailIsValid}] [referralEmail: ${referralEmail} valid: ${refferalEmailIsValid}]`
      );
      res.status(400);
      res.send(JSON.stringify({ errorCode: errorCodes.emailIsNotValid }));
      return;
    ...
    if (!isUserRegistered) {
      logger.warning(`checkReferral user is not registered [Email: ${email}]`);
      res.status(400);
    ...
    if (isReferralSet) {
      logger.warning(
        `checkReferral user already received refferal [Email: ${email}]`
      );
      res.status(400);
      res.send(
        JSON.stringify({ errorCode: errorCodes.refferalAlreadyReceivedBonus })
      );
      return;
    }
    ...
    logger.error(`214 checkReferral result: ${error} Stack: ${error.stack}`);
    res.status(500);
    res.send("" + error);
  }
});
